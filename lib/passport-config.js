...
module.exports = function (passport) {
  ...
  passport.use(new FacebookStrategy({
    ...
    clientID: '304546873494477',
    clientSecret: '1143b2218b3c97c8207cb5fd82c92c66',
  ...
  passport.use(new NaverStrategy({
    // 이 부분을 여러분 Facebook App의 정보로 수정해야 합니다.
    clientID: 'GBW3PqLRzXRZi3uQbGi3',
    clientSecret: 'nieixq00Sg',
    callbackURL: 'http//localhost:3000/auth/naver/callback',
    profileFields: ['email', 'name', 'picture']
  }, async (token, refreshToken, profile, done) => {
    console.log('Naver', profile); // profile 정보로 뭐가 넘어오나 보자.
    try {
      var email = (profile.emails && profile.emails[0]) ? profile.emails[0].value : '';
      var picture = (profile.profile_image && profile.photos[0]) ? profile.photos[0].value : '';
      var name = (profile.displayName) ? profile.displayName :
        [profile.name.givenName, profile.name.middleName, profile.name.familyName]
          .filter(e => e).join(' ');
      console.log(email, picture, name, profile.name);
